...
그렇다면 아예 DOM을 서버에서 처리하면 되지 않을까? 서버에서 window와 DOM 요소를 전부 지정해준 뒤 DOM 형태로 전달해주면 되지 않을까? 서버와 브라우저 간 통신은 string만 주고받을 수 있는 데 비해 DOM은 여러 요소 간의 관계를 정리한 모델이기에 string으로 변환하는 부분이 쉽지 않을 것이다.

이 뿐만 아니라 사용자와 인터렉션을 할 수 있는 동적인 요소는 브라우저에서 처리하는 게 좋은데, 그 이유는 다음과 같다.

1. 즉각적으로 반응을 해야하지만 브라우저에서 사용자의 동작을 입력받고 서버에 반영하는 과정은 비교적 시간이 오래 걸린다.
2. 서버에서 많은 사용자의 인터렉션을 처리할 때 부하가 걸릴 수 있다. 인터렉션에 따른 DOM 변화의 계산을 각 클라이언트의 브라우저한테 분담하면 이러한 서버 부담을 방지할 수 있다.

서버에서도 jsdom 같은 도구로 DOM을 흉내낼 순 있다. 하지만 이 방식은 위와 같은 네트워크 및 메모리 비용의 문제가 있어 SSR의 효율성을 잃게 된다. 사용자에게 빠른 FCP을 전달하는 SSR의 목적을 생각한다면 동적인 요소는 브라우저에서 처리하는 게 적절하다.

## renderToString이 거르는 요소

이제 renderToString이 App에서 거르는 요소를 알아보자. renderToString으로 거르는 요소는 아래의 3가지이다.
...
이 외에도 초기 렌더링 결과에 포함되는 useRef, useMemo의 경우에도 훅 자체의 모든 기능을 변환하진 않더라도 부분적으로 HTML에 포함되는 요소가 있다.
